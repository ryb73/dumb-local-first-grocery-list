...
*   **Atomicity:** The atomicity of writes between the primary application database and the `groceries.log.sqlite3` database is a critical concern (as noted in `PRIMARY.md`). Strategies to ensure that an operation updates both databases or neither will need to be carefully implemented, especially since they are separate database files. This was an area for further investigation and testing.
    *   **Update (YYYY-MM-DD):** Initial testing using `sqlocal` (for OPFS-based SQLite in WASM) and Kysely has confirmed that SQLite's standard transaction mechanisms, when combined with `ATTACH DATABASE`, can ensure atomicity across two separate database files. A Kysely transaction initiated on the primary database connection (after attaching the secondary database) successfully rolls back changes in *both* database files if an error occurs at any point during the transaction. This demonstrates that operations intended to be atomic across the main application database and the operation log database can be reliably implemented. The `experimentation/multi-db-atomic-test` directory contains the demonstration and test code for this verification.
